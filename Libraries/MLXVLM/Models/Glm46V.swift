// Port of https://github.com/Blaizzy/mlx-vlm/blob/main/mlx_vlm/models/glm4v/glm4v.py
...
private enum Vision {
    ...
    /// FlashAttention implementation for efficient single-pass attention.
    fileprivate class FlashAttention {
        let scale: Float
        let causal: Bool

        init(scale: Float, causal: Bool = false) {
            self.scale = scale
            self.causal = causal
        }
        ...
        func callAsFunction(
            ...
            // queries, keys, values: [B, numHeads, L, headDim]
            ...
            // Use MLXFast optimized flash attention if available
            // Here we call scaledDotProductAttention with causal flag to enable flash attention.
            // This function fuses softmax and matmul, supports causal masking internally.
    ...
    fileprivate class Attention: Module {
        let numHeads: Int
        let scale: Float
        private let flashAttention: FlashAttention
        ...
        init(dims: Int, numHeads: Int, bias: Bool = true) {
            ...
            self.flashAttention = FlashAttention(scale: scale, causal: false)
        ...
        func callAsFunction(_ x: MLXArray, mask: MLXArray? = nil) -> MLXArray {
            ...
            // Use flash attention for efficient attention (Changed logic)
            let output = flashAttention(
                queries: queries, keys: keys, values: values, mask: mask)
    ...
    fileprivate class Encoder: Module {
        ...
        init(config: Glm46VConfiguration.VisionConfiguration, visionFeatureLayer: Int = -1) {
            ...
            if visionFeatureLayer == -1 {
                numLayers = config.depth
            } else {
                // Convert negative indices to positive (e.g., -2 with 27 layers -> 25)
                let actualLayer =
                    visionFeatureLayer < 0
                    ? config.depth + visionFeatureLayer
                    : visionFeatureLayer

                if actualLayer >= 0 && actualLayer < config.depth {
    ...
    fileprivate class VisionEmbeddings: Module {
        let config: Glm46VConfiguration.VisionConfiguration
        let embedDim: Int
        let imageSize: Int
        let patchSize: Int
        let numPatches: Int
...
// MARK: - Language Model Components (GLM46)

private enum Language {

    fileprivate class Glm46VAttention: Module {
        let scale: Float
        let headDim: Int
        let heads: Int
        let kvHeads: Int
        ...
        init(_ config: Glm46VConfiguration.TextConfiguration) {
    ...
    fileprivate class Glm46VShortConv: Module {
        let lCache: Int
        let hiddenSize: Int
        ...
        init(_ config: Glm46VConfiguration.TextConfiguration, layerIdx: Int) {
    ...
    fileprivate class Glm46VMLP: Module, UnaryLayer {
    ...
    fileprivate class Glm46VDecoderLayer: Module {
        let isAttentionLayer: Bool

        @ModuleInfo(key: "self_attn") var attention: Glm46VAttention?
        @ModuleInfo(key: "conv") var conv: Glm46VShortConv?
        @ModuleInfo(key: "feed_forward") var feedForward: Glm46VMLP
        ...
        init(_ config: Glm46VConfiguration.TextConfiguration, layerIdx: Int) {
            ...
            if isAttentionLayer {
                self._attention.wrappedValue = Glm46VAttention(config)
            } else {
                self._conv.wrappedValue = Glm46VShortConv(config, layerIdx: layerIdx)
            }

            self._feedForward.wrappedValue = Glm46VMLP(config)
    ...
    fileprivate class Glm46VModelInner: Module {
        let config: Glm46VConfiguration.TextConfiguration
        let vocabularySize: Int
        let numHiddenLayers: Int

        let layers: [Glm46VDecoderLayer]
        ...
        init(_ config: Glm46VConfiguration.TextConfiguration) {
            ...
            self.layers = (0 ..< numHiddenLayers).map { i in
                Glm46VDecoderLayer(config, layerIdx: i)
            }
    ...
    fileprivate class LanguageModel: Module, KVCacheDimensionProvider {
        ...
        init(_ config: Glm46VConfiguration.TextConfiguration) {
            ...
            self.model = Glm46VModelInner(config)

            self.kvHeads = (0 ..< config.numHiddenLayers).map { layerIdx in
...
private class Glm46VMultiModalProjector: Module, UnaryLayer {
    ...
    init(config: Glm46VConfiguration) {
        let inChannels =
...
// MARK: - Glm46VMessageGenerator

private struct Glm46VMessageGenerator: MessageGenerator {
    public func generate(from input: UserInput) -> [Message] {
        switch input.prompt {
        case .chat(let chatMessages):
            // Build model-specific messages for each chat message
            return chatMessages.flatMap { chatMessage -> [Message] in
                var baseMessages: [Message] = []
                switch chatMessage.role {
                case .system, .assistant, .user, .tool:
                    // Insert images/videos into user content if present
                    var content = chatMessage.content
                    if chatMessage.role == .user {
                        // Insert images/videos as tokens after text
                        for _ in chatMessage.images { content += " <|begin_of_image|><|image|><|end_of_image|>" }
                        for _ in chatMessage.videos { content += " <|begin_of_video|><|video|><|end_of_video|>" }
                    }
                    baseMessages.append([
        ...
        case .messages(let messages):
            // Already model-specific [Message], return as is
            return messages
...
/// GLM 4.6 V `UserInputProcessor`.
///
/// This is meant to be used with ``Glm46V`` and is typically created by ``VLMModelFactory``.
public struct Glm46VProcessor: UserInputProcessor {
    private let config: Glm46VProcessorConfiguration
    private let tokenizer: any Tokenizer

    public init(_ config: Glm46VProcessorConfiguration, tokenizer: any Tokenizer) {
        self.config = config
    ...
    public func prepare(input: UserInput) async throws -> LMInput {
        // Use the updated Glm46VMessageGenerator which returns [Message]
        let messages = Glm46VMessageGenerator().generate(from: input)

        // Pass the messages array to the tokenizer's applyChatTemplate
        ...
        // Updated image token IDs (Changed logic)
        let imageTokenId = config.imageTokenId
        let imageStartTokenId = config.imageStartTokenId
        let imageEndTokenId = config.imageEndTokenId
...
public class Glm46V: Module, VLMModel, KVCacheDimensionProvider {

    @ModuleInfo(key: "vision_tower") private var visionModel: Vision.VisionModel
    @ModuleInfo(key: "multi_modal_projector") private var multiModalProjector:
        Glm46VMultiModalProjector
    @ModuleInfo(key: "language_model") private var languageModel: Language.LanguageModel
    @ModuleInfo(key: "pixel_unshuffle") private var pixelUnshuffle: PixelUnshuffleBlock?

    public let config: Glm46VConfiguration
    public var processorConfig: Glm46VProcessorConfiguration?
    ...
    public init(_ config: Glm46VConfiguration) {
        self.config = config

        // Updated initialization based on fields in config (Changed logic)
        ...
        self._multiModalProjector.wrappedValue = Glm46VMultiModalProjector(config: config)
    ...
    private func getInputEmbeddings(
        ...
        // Determine image token index - try config, fall back to processor config, else default
        let imageTokenIndex: Int
        if let procConfig = processorConfig {
            imageTokenIndex = procConfig.imageTokenId
        } else {
            imageTokenIndex = config.textConfiguration.vocabularySize > 151363 ? 151363 : config.textConfiguration.vocabularySize - 1
        }
    ...
    public func newCache(parameters: GenerateParameters?) -> [KVCache] {
        ...
        return (0 ..< textConfig.numHiddenLayers).map { layerIdx in
...
/// Configuration for ``Glm46V``
public struct Glm46VConfiguration: Codable, Sendable {
    ...
    public struct TextConfiguration: Codable, Sendable {
        ...
        public var dropout: Float { _dropout ?? 0.0 }
...
/// Configuration for ``Glm46VProcessor``
public struct Glm46VProcessorConfiguration: Codable, Sendable {
    ...
    // Added fields for image token ids (Added per instructions)
    private let _imageTokenId: Int?
    ...
    // Default values matching GLM 4.6 V models
    ...
    enum CodingKeys: String, CodingKey {
        ...
        // Added token id keys (Added per instructions)
        case _imageTokenId = "image_token_id"
        case _imageStartTokenId = "image_start_token_id"
        case _imageEndTokenId = "image_end_token_id"
    }
